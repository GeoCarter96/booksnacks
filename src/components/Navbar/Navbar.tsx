...
const [showResults, setShowResults] = useState(false);
  ...
  const delayDebounce = setTimeout(async () => {
    ...
    try {
      ...
      console.log("API Response:", data); // Better debug
...
<input className='search__input' placeholder='Search For Books' type='text' value={search} onChange={(e) => setSearch(e.target.value)}  onFocus={() => setShowResults(true)}
  ...
  <div className="search-dropdown">
    {results.map((item) => (
      <Link 
        ...
        onClick={() => setResults([])} 
      >
        <div style={{ color: 'red' }}>Found: {item?.title || "Unknown Book"}</div>

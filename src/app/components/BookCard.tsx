...
import type { Product } from '../lib/books';



export default function BookCard({ product }: { product: Product}){
    return(
        <Link className='for-you__recommended--books-link' href={`/book/${product.id}`}>
<audio src={product.audioLink}></audio>
<figure className='book__image--wrapper' >
<img className="book__image" src={product.audioLink} alt="book" ></img>
</figure>
<div className='recommended__book--title'>{product.title}</div>
<div className='recommended__book--author'>{product.author}</div>
<div className='recommended__book--sub-title'>{product.subTitle}</div>
...
<div className="recommended__book--details-text">{product.averageRating}</div>
</div>
</div>
</Link>
    )
